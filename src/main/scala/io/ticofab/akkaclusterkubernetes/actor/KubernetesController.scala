  ...
  override def receive = {
    ...
    case AddNode =>
      ...
      if (workers.get != null) {
        ...
        if (replicas < 2) {
          println(s"Scaling up Deployment $workerDeploymentName to $replicas replicas")
        ...
        println(s"Creating new Deployment $workerDeploymentName")
    ...
    case RemoveNode =>
      ...
      if (workers.get != null) {
        ...
        if (replicas >= 1) {
          println(s"Scaling down Deployment $workerDeploymentName to $replicas replicas")

          workers.scale(replicas)

        } else {
          println("Only one replica remains in the Deployment - not scaling down")
        }
      } else {
        println("Deployment doesn't exist, not scaling down")
      }
